...
mod test {
    ...
    use crate::{
        live_builder::constraint_client::ConstraintSubscriber, mev_boost::RelayClient,
    ...
    async fn test_constraint_subscriber() {
        ...
        let relay_client = RelayClient::from_url(endpoint.parse().unwrap(), None, None, None);
        let relays = vec![MevBoostRelaySlotInfoProvider::new(
            relay_client,
            "preconf relay".to_string(),
        )];
